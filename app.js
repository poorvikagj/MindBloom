    ...
    connection.query(userQuery, [userId], (err, userResult) => {
        ...
        connection.query(enrolledCoursesQuery, [userId], (err, coursesResult) => {
            ...
            res.render('listings/profile.ejs', {
                ...
                currentUser: req.session.user // Fixed: use req.session.user instead of req.user
            });
        });
    });
});
...
// Update course - FIXED: Changed from /courses/:id to /course/:id
// 

app.put('/mindbloom/course/:id', requireAdmin, (req, res) => {
    const courseId = req.params.id; // Keep original course ID
    const { title, description, video, tname, imglink, vidLink } = req.body;
    const teacherValidationQuery = `SELECT TID FROM TEACHER WHERE TNAME = ?`;
    ...
    connection.query(teacherValidationQuery, [tname], (err, teacherResult) => {
        ...
        // Teacher exists, get the TID and proceed with course update
        const teacherId = teacherResult[0].TID;
        // Don't generate new courseId - use the one from params!
        ...
        connection.query(updateQuery, [title, description, video, teacherId, imglink, vidLink, courseId], (err, result) => {
            if (err) {
                ...
                return res.redirect(`/mindbloom/course/${courseId}/edit`);
            }
...
// Delete course - FIXED: Changed from /courses/:id to /course/:id
app.delete('/mindbloom/course/:id', requireAdmin, (req, res) => {
    const courseId = req.params.id;

    // First delete all enrollments for this course
    const deleteEnrollmentsQuery = `DELETE FROM ENROLLMENT WHERE CID = ?`;

    connection.query(deleteEnrollmentsQuery, [courseId], (err) => {
        if (err) {
            console.log(err);
            req.flash('error', 'Error deleting course enrollments.');
            return res.redirect('/mindbloom');
        }

        // Then delete the course
        const deleteCourseQuery = `DELETE FROM COURSE WHERE CID = ?`;

        connection.query(deleteCourseQuery, [courseId], (err, result) => {
            if (err) {
                console.log(err);
...
// ============ TEACHER CRUD ROUTES (ADMIN ONLY) ============

// Add new teacher form
// app.get('/mindbloom/teachers/new', requireAdmin, (req, res) => {
//     res.render('listings/newteacher.ejs');
// });

// // Create new teacher
// app.post('/mindbloom/teachers', requireAdmin, (req, res) => {
//     const { name, email, expertise, bio } = req.body;
//     const teacherId = faker.string.uuid();

//     const insertQuery = `INSERT INTO TEACHER (TID, TNAME, EMAIL, EXPERTISE, BIO) VALUES (?, ?, ?, ?, ?)`;

//     connection.query(insertQuery, [teacherId, name, email, expertise, bio], (err, result) => {
//         if (err) {
//             console.log(err);
//             req.flash('error', 'Error creating teacher.');
//             return res.redirect('/mindbloom/teachers/new');
//         }

//         req.flash('success', 'Teacher created successfully!');
//         res.redirect(`/mindbloom/teachers/${teacherId}`);
//     });
// });

// // Get edit form for teacher
// app.get('/mindbloom/teachers/:id/edit', requireAdmin, (req, res) => {
//     const teacherId = req.params.id;

//     const teacherQuery = `SELECT * FROM TEACHER WHERE TID = ?`;

//     connection.query(teacherQuery, [teacherId], (err, result) => {
//         if (err) {
//             console.log(err);
//             req.flash('error', 'Error fetching teacher details.');
//             return res.redirect('/mindbloom');
//         }
...
//         res.render('listings/editteacher.ejs', { teacher: result[0] });
//     });
// });

// // Update teacher
// app.put('/mindbloom/teachers/:id', requireAdmin, (req, res) => {
//     const teacherId = req.params.id;
//     const { name, email, expertise, bio } = req.body;

//     const updateQuery = `UPDATE TEACHER SET TNAME = ?, EMAIL = ?, EXPERTISE = ?, BIO = ? WHERE TID = ?`;

//     connection.query(updateQuery, [name, email, expertise, bio, teacherId], (err, result) => {
//         if (err) {
//             console.log(err);
//             req.flash('error', 'Error updating teacher.');
//             return res.redirect(`/mindbloom/teachers/${teacherId}/edit`);
//         }
...
// // Delete teacher
// app.delete('/mindbloom/teachers/:id', requireAdmin, (req, res) => {
//     const teacherId = req.params.id;

//     // Check if teacher has courses
//     const checkCoursesQuery = `SELECT COUNT(*) as courseCount FROM COURSE WHERE TID = ?`;

//     connection.query(checkCoursesQuery, [teacherId], (err, result) => {
//         if (err) {
//             console.log(err);
//             req.flash('error', 'Error checking teacher courses.');
//             return res.redirect('/mindbloom');
//         }

//         if (result[0].courseCount > 0) {
//             req.flash('error', 'Cannot delete teacher. They have active courses. Please reassign or delete courses first.');
//             return res.redirect(`/mindbloom/teachers/${teacherId}`);
//         }

//         // Delete teacher if no courses
//         const deleteTeacherQuery = `DELETE FROM TEACHER WHERE TID = ?`;

//         connection.query(deleteTeacherQuery, [teacherId], (err, result) => {
//             if (err) {
//                 console.log(err);
...
// Root route
app.get("/", (req, res) => {
    res.send("Home page of online course");
});
